...
en_fall_2020 <- raw %>%
  select('Course ID', 'Course Title', 'Course Name', UGrad) %>%
  filter (UGrad > 10) %>%
  arrange(desc(UGrad)) %>%
  clean_names()
...
View(gov50)
```
Class with highest enrollment is EC10. GOV 50 has enrollment of 110 and 41.
...
```{r}
top10_course <- en_fall_2020 %>%
  slice(2:11)
#View(top10_course)

ggplot(top10_course, aes(x = fct_reorder(course_title, u_grad), y = u_grad)) +
  geom_col() +
  labs(title = 'Largest 10 Harvard Undergrad Courses', 
       x = 'Course Title', 
       y = 'Enrollment') + 
  coord_flip() 
```
